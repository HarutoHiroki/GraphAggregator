...
export function validatePhoneBookShape(json) {
  ...
  for (let i = 0; i < Math.min(json.length, 50); i++) {
    ...
    if (!Array.isArray(brand.phones)) errors.push(`brand[${i}].phones not an array`);
      ...
      for (let j = 0; j < Math.min(brand.phones.length, 5); j++) {
        const p = brand.phones[j];
        if (typeof p?.name !== 'string') errors.push(`brand[${i}].phones[${j}].name missing`);
        if (typeof p?.file !== 'string' && !Array.isArray(p?.file)) {
          errors.push(`brand[${i}].phones[${j}].file must be string or array`);
        }
      }
    }
  }
