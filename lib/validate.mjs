...
// Strip C0/C1 control chars and trim to a safe upper bound. Returns null for
// non-strings or values that become empty after cleaning. Use on anything from
// a third-party JSON source that ends up in logs or in the published index.
...
// is third-party content with no PR review, so we validate every field we
